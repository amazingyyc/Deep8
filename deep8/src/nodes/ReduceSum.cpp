...
template <typename T>
ReduceSum<T>::ReduceSum(std::vector<Node*> &inputs, int a, bool keep) : Function<T>(inputs), axis(a), keepDims(keep) {
	check();
}

template <typename T>
void ReduceSum<T>::check() {
	Function<T>::check();
...
template <typename T>
void ReduceSum<T>::forwardCPU(const std::vector<const Tensor<T>*> &inputs, Tensor<T> *output) {
    ...
    y.device(*device) = x.sum(reduceDims).reshape(reshape);
}

template <typename T>
void ReduceSum<T>::backwardCPU(const std::vector<const Tensor<T>*> &inputs, const Tensor<T> *output, const Tensor<T> *outputGradient, size_t index, Tensor<T> *iGradient) {
    ...
    dx.device(*device) += dy.broadcast(broad);
}

DEEP8_RE_DECLARATION_HALF_FUNC(ReduceSum);
DEEP8_DECLARATION_INSTANCE(ReduceSum)
