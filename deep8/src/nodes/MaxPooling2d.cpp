...
void MaxPooling2d<T>::check() {
	...
	DEEP8_ARGUMENT_CHECK(3 == this->inputs[0]->outputShape.nDims, "MaxPooling2d needs inputs nDims is 3");
...
template <typename T>
void MaxPooling2d<T>::forwardCPU(const std::vector<const Tensor<T>*> &inputs, Tensor<T> *output) {
...
template <typename T>
void MaxPooling2d<T>::backwardCPUImpl(T *input,
									T *inputGrad,
									T *outputGrad,
									int64_t batch,
...
#ifdef HAVE_HALF
template <>
void MaxPooling2d<half>::backwardCPUImpl(half *input,
				half *inputGrad,
				half *outputGrad,
				int64_t batch,
				int64_t startChannel,
...
template <typename T>
void MaxPooling2d<T>::backwardCPU(const std::vector<const Tensor<T>*> &inputs,
