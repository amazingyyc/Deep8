...
void ReduceSum(const Tensor& x, Tensor& y, const std::vector<int>& axis, bool keepDims) {
    ...
    if (DeviceType::CPU == x.deviceType()) {
        ReduceSumCPU(x, y, axis, keepDims);
    } else {
#ifdef HAVE_CUDA
        ReduceSumGPU(x, y, axis, keepDims);
#else
...
void ReduceSumGrad(const Tensor& x, Tensor& dx, const Tensor& y, const Tensor& dy, const std::vector<int>& axis, bool keepDims) {
    ...
    if (DeviceType::CPU == x.deviceType()) {
        ReduceSumGradCPU(x, dx, y, dy, axis, keepDims);
    } else {
#ifdef HAVE_CUDA
        ReduceSumGradGPU(x, dx, y, dy, axis, keepDims);
#else
        DEEP8_RUNTIME_ERROR("do not have a GPU");
#endif  
    }
...
void ReduceSumCPUEigenImpl(CPUDevice* device, T* x, std::vector<int>& xarray, T* y, std::vector<int>& yarray) {
    ...
    yvec.reshape(yshape).device(*eigenDevice) = xvec.reshape(xshape).sum(reduceDims).reshape(yshape);
}


template <typename T>
void ReduceSumCPUImpl(CPUDevice *device, T *x, const Shape &xshape, T *y, const Shape &yshape, const std::vector<int>& axis) {
    ...
    if (1 == NumDims) {
        if (0 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 1, 0>(device, x, xarray, y, yarray);
        } else if (1 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 1, 1>(device, x, xarray, y, yarray);
        } else {
            DEEP8_RUNTIME_ERROR("the shape is error");
        }
    } else if (2 == NumDims) {
        if (0 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 2, 0>(device, x, xarray, y, yarray);
        } else if (1 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 2, 1>(device, x, xarray, y, yarray);
        } else if (2 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 2, 2>(device, x, xarray, y, yarray);
        } else {
            DEEP8_RUNTIME_ERROR("the shape is error");
        }
    } else if (3 == NumDims) {
        if (0 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 3, 0>(device, x, xarray, y, yarray);
        } else if (1 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 3, 1>(device, x, xarray, y, yarray);
        } else if (2 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 3, 2>(device, x, xarray, y, yarray);
        } else if (3 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 3, 3>(device, x, xarray, y, yarray);
        } else {
            DEEP8_RUNTIME_ERROR("the shape is error");
        }
    } else if (4 == NumDims) {
        if (0 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 4, 0>(device, x, xarray, y, yarray);
        } else if (1 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 4, 1>(device, x, xarray, y, yarray);
        } else if (2 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 4, 2>(device, x, xarray, y, yarray);
        } else if (3 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 4, 3>(device, x, xarray, y, yarray);
        } else if (4 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 4, 4>(device, x, xarray, y, yarray);
        } else {
            DEEP8_RUNTIME_ERROR("the shape is error");
        }
    } else if (5 == NumDims) {
        if (0 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 5, 0>(device, x, xarray, y, yarray);
        } else if (1 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 5, 1>(device, x, xarray, y, yarray);
        } else if (2 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 5, 2>(device, x, xarray, y, yarray);
        } else if (3 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 5, 3>(device, x, xarray, y, yarray);
        } else if (4 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 5, 4>(device, x, xarray, y, yarray);
        } else if (5 == ReduceCount) {
            ReduceSumCPUEigenImpl<T, 5, 5>(device, x, xarray, y, yarray);
        } else {
            DEEP8_RUNTIME_ERROR("the shape is error");
        }
    } else {
...
void ReduceSumGradCPUEigenImpl(CPUDevice* device, T* x, T* dx, std::vector<int>& xarray, T* y, T* dy, std::vector<int>& yarray) {
    auto eigenDevice = device->eigenDevice;
    ...
    dxvec.reshape(xshape).device(*eigenDevice) += dyvec.reshape(yshape).broadcast(broadDims);
}

template <typename T>
void ReduceSumGradCPUImpl(CPUDevice*device, T *x, T *dx, const Shape &xshape, T *y, T *dy, const Shape &yshape, const std::vector<int>& axis) {
    auto xarray = convertShapeToVector(xshape);
    ...
    if (1 == NumDims) {
        ReduceSumGradCPUEigenImpl<T, 1>(device, x, dx, xarray, y, dy, yarray);
    } else if (2 == NumDims) {
        ReduceSumGradCPUEigenImpl<T, 2>(device, x, dx, xarray, y, dy, yarray);
    } else if (3 == NumDims) {
        ReduceSumGradCPUEigenImpl<T, 3>(device, x, dx, xarray, y, dy, yarray);
    } else if (4 == NumDims) {
        ReduceSumGradCPUEigenImpl<T, 4>(device, x, dx, xarray, y, dy, yarray);
    } else if (5 == NumDims) {
        ReduceSumGradCPUEigenImpl<T, 5>(device, x, dx, xarray, y, dy, yarray);
    } else {
        DEEP8_RUNTIME_ERROR("the shape is error");
    }
}
